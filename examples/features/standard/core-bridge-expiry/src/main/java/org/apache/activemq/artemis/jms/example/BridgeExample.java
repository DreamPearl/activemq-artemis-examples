...
/**
 * This example demonstrates a core bridge set-up between two nodes, by setting expiry on producer and fetching it from expiry queue..
 */
public class BridgeExample {
   ...
   public static void main(final String[] args) throws Exception {
      ...
      try {
         ...
         // Sleep for 5s. Once we wake up, the message will have been expired
         System.out.println("Sleep a little bit to let the message expire...");
         Thread.sleep(5000);


         // Trying to receive a message. Since there is none on the queue, the call will timeout after 5000ms
         // and messageReceived will be null
         TextMessage messageReceived = (TextMessage) consumer.receive(5000);
         System.out.println("Received message from node 0 queue " + sausageFactory.getQueueName() + ": " + messageReceived);
         System.out.println("Received message from node 1 queue " + mincingMachine.getQueueName() + ": " + messageReceived);

         // Expiry queue on ic0 [TODO]
         Queue expirySausageQueue = (Queue) ic0.lookup("queue/expiry-sausage-Queue");
         MessageConsumer expiryConsumer0 = session1.createConsumer(expirySausageQueue);
         messageReceived = (TextMessage) expiryConsumer0.receive(5000);
         System.out.println("Received message from " + expirySausageQueue.getQueueName() + ": " + messageReceived.getText());

         // Expiry queue on ic1 [TODO]
         Queue expiryMincingQueue = (Queue) ic1.lookup("queue/expiry-mincing-Queue");
         MessageConsumer expiryConsumer1 = session1.createConsumer(expiryMincingQueue);
         messageReceived = (TextMessage) expiryConsumer1.receive(5000);
         System.out.println("Received message from " + expiryMincingQueue.getQueueName() + ": " + messageReceived.getText());
      } finally {
